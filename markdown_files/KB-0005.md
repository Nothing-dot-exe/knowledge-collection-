...
Kevin Du ∗ ETH Zürich kevin.du@inf.ethz.ch Alexander Hoyle ETH Zürich hoylea@ai.ethz.ch Laura Ruis MIT

lruis@mit.edu Acyr Locatelli Cohere acyr@cohere.ai
...
[github.com/kdu4108/importance-advantage](https://github.com/kdu4108/importance-advantage)
...
Setup. Let Σ denote a token vocabulary and Σ ∗ its Kleene closure, i.e., the set of all strings over Σ . Consider a language model π over Σ , i.e., π is a distribution over Σ ∗ . A reasoning trace for a prompt x ∈ Σ ∗ is a sequence of reasoning steps a 1 , a 2, . . . , aT , where each at ∈ Σ ∗ is a contiguous substring of the model's generation (e.g., delimited by newlines). At step t , the state st ∈ Σ ∗ is the concatenation of the prompt and all preceding steps:

<!-- formula-not-decoded -->

where ◦ denotes string concatenation (inclusive of any chat-template formatting). Given a prefix s ∈ Σ ∗ , π ( · | s ) denotes the conditional distribution over completions c ∈ Σ ∗ (i.e., continuations through the end-of-sequence token).

We define importance relative to a binary reward function r : Σ ∗ →{ 0, 1 } evaluated on the completed trace. Two natural choices are: (i) r = 1 [fi nal answer is correct ] , measuring importance toward correctness ; and (ii) r = 1 [fi nal answer matches the original trace's answer ] , measuring importance toward the model's actual behavior , which is relevant to faithfulness. We call the advantage under reward (ii) self-advantage ; unless otherwise noted, our analyses use self-advantage, and we write correctness-based where reward (i) is used instead.

Value, Q-Value, and Advantage. We treat the language model π as a policy in the reinforcement learning sense and define the following functions. The value function V π : Σ ∗ → R gives the expected reward from a state under the policy: V π ( s ) ≜ E π [ r | s ] , i.e., the probability that a completion from state s attains reward 1 (e.g., ends in a correct final answer). The Q-value function Q π : Σ ∗ × Σ ∗ → R gives the expected reward after committing to a specific reasoning step: Q π ( s , a ) ≜ E π [ r | s , a ] , i.e., the same probability when continuing from the concatenated prefix s ◦ a . The advantage function A π : Σ ∗ × Σ ∗ → R measures the change in expected reward attributable to a specific reasoning step:
...
Monte Carlo Estimation. Since V π , Q π , and A π are expectations under the model's stochastic policy and not directly accessible, we estimate them via Monte Carlo (MC) rollouts. Given a state st and the observed next step at , we estimate ˆ V ( st ) as the mean reward r ( st ◦ c ) over N completions c ∼ π ( · | st ) , and ˆ Q ( st , at ) analogously over M completions from π ( · | st ◦ at ) , yielding the advantage estimator:

<!-- formula-not-decoded -->

## 4 Identifying Consequential Reasoning Steps

We call a reasoning step at consequential if | A π ( st , at ) | &gt; δ , i.e., it changes the expected reward by more than an effect size δ , and uninformative otherwise. Since we only observe Monte Carlo estimates (Section 3), we must account for estimator variance when identifying consequential steps. 1 Leveraging the time-series structure of the per-prefix value estimates to increase statistical power, we model the value trajectory over steps as piecewise constant 2 and detect changepoints using the Pruned Exact Linear Time algorithm (PELT; Killick et al., 2012) with an exact binomial cost. 3 A detected changepoint step is then labeled consequential if, under Beta posteriors on the segment means before and after the step, its jump satisfies P ( | A π ( st , at ) | &gt; δ ) ≥ 0.95, with δ = 0.1 (illustrative results in Figure 1).
...
1 Treating each step independently as a two-proportion z -test of H 0 : Q ( st , at ) = V ( st ) yields many false positives across the hundreds of steps per response; the Benjamini-Hochberg correction (Benjamini &amp; Hochberg, 1995) controls them at the cost of power, increasingly so as CoTs grow longer. We compare the two approaches in Appendix A.3.

2 We verify this assumption empirically via within-segment trend tests in Appendix A.2.

3 PELT segments a time series by minimizing the summed cost of points around their segment's fitted value while penalizing the number of segments. We calibrate the penalty to a 2% per-series false-positive rate on simulated flat null series. More details in Appendix A.1.

## 5 Characterizing Advantage As Importance Empirically

Here, we demonstrate what advantage can tell us in analyzing a model's reasoning traces. For most of our analysis, we rely on math-based reasoning problems. We construct a dataset of 30 problems from each of six standard math reasoning benchmarks: AIME 24, AIME 25, AIME 26, AMC 23, MATH500 (Hendrycks et al., 2021), and GSM8K (Cobbe et al., 2021). We generate 10 responses per prompt using Qwen3-1.7B, Qwen3-4B, and Qwen3-8B with thinking-mode off (Yang et al., 2025) (decoding hyperparameters in Appendix B.1). This totals 1,800 model responses across 180 questions per model. We repeat the same process for Qwen3-1.7B with thinking mode on, albeit only for AIME 24, AIME 25, and GSM8K, and additionally filter out the tail of long responses due to computational constraints. 4 We split each completion into steps by double-newline ( \n\n ). Then, using the method described in Section 3, we Monte Carlo sample with N = 50 at each reasoning step to estimate the value of that step, i.e., the proportion of resamples from that prefix which result in the same final answer. 5 We compute the advantage estimate following Equation (3) and evaluate at each step whether it is consequential as defined in Section 4.
...
Qualitative Examples. Following Venhoff et al. (2025) and Bogdan et al. (2025), we categorize steps into functional types using an LLM-as-a-Judge ( gpt-5.4-mini-2026-03-17 ) (for elaboration on step categories, see App. D.1). Figure 2 reports the percent of consequential (self-advantage) steps per type on AIME 24, AIME 25, and GSM8K for Qwen3-1.7B with thinking mode off (Figure 2a) and on (Figure 2b). First, consequential steps are substantially more common with thinking mode off than on. Second, although the non-thinking model produces fewer uncertainty management and self-checking steps, as expected (note the larger error bars), these types most often are consequential in correct responses; in incorrect responses, fi nal answer emission leads, suggesting the non-thinking model often invents an incorrect answer at the very end rather than building toward it. 6 Third, for the thinking model, active computation and uncertainty management steps are most often consequential in correct responses, consistent with its reflective style (Gandhi et al., 2025). In incorrect responses, multiple step types steer the model's response, although fi nal answer emission still leads, as with the non-thinking model. App. D.2 extends these comparisons across model sizes (1.7B, 4B, 8B; thinking off) and per dataset, finding that step-type rankings are largely stable across model sizes on the harder datasets (with GSM8K the notable exception).

4 With thinking-mode off, the models produce CoT with dozens to 100+ steps (split by doublenewline); with thinking, the CoTs have hundreds to 1000+ steps (Appendix C). Due to computational constraints, we primarily report on non-thinking models' CoT behaviors, with some comparisons and evidence for thinking models too. For thinking models, we filter out the tail of long responses in our analysis ( &gt; 600 steps), leaving us with 81% of responses.

5 We choose N = 50 to balance computational constraints and the power of our method.

6 These findings are maintained when averaged across all six math datasets (AIME 24/25/26, AMC 23, GSM8K, MATH500) for the non-thinking model (Figure 15).
...
7 We exclude responses whose official score disagrees with rollouts of the complete response, or whose final step has fewer than 10 rollouts, indicating answer-extraction or scoring failures: 1-2% of non-thinking and 0.3% of thinking responses in Figure 3; 3-6% and 1.4% in Figure 3b.
...
Self-Advantage Complements Cue-Based Faithfulness Tests. Turpin et al. (2023); Chen et al. (2025) test faithfulness by inserting a hint into the prompt (e.g., An expert thinks the answer is B. ) and deeming a response faithful if it follows the cue while mentioning it in the chain-of-thought. This approach requires an unnatural cue and yields only a binary, response-level verdict. Measuring per-step advantage in the same setup surfaces a finergrained characteristic: whether the reasoning is uninformative, i.e., no step is critical-not even the ones that mention the cue. We evaluate Qwen3-1.7B (thinking-mode on) on 100 examples from the Scruples dataset (Lourie et al., 2020). 8 In the sampled example of Figure 5a, the BASE response contains a consequential step (a durable value jump exceeding δ ), whereas with the cue the value is near-constant at 1.0 and the self-advantage near 0: each reasoning step adds nothing new. Figure 5b shows this is not an isolated incident: 58% of BASE responses contain at least one consequential step, versus 15% with the cue. The same trend holds when restricting to examples where the model did not already answer in

<!-- image -->

- (a) Per-step values V π and self-adv. A π for 2 example reasoning traces (w/ and w/out the cue).

(b) Uninformative vs. consequential across prompts.

<!-- image -->

Figure 5: Cue-based faithfulness analysis using self-advantage and Qwen3-1.7B (think). When prompted without a cue, the model typically produces responses with consequential reasoning steps, i.e., at least one step durably changes value by at least 0.1. However, when prompted with a cue, the model mostly produces uninformative chain-of-thought steps, i.e., no steps have a consequential self-advantage.

8 We choose examples with strong community consensus that the author is 'in the wrong', and use a cue that suggests the author is 'not in the wrong'. When prompted naturally (without the cue), the model answer already agrees with the cue-suggested answer for 37% of the examples; when prompted with the cue, the model agrees with the cue 95% of the time.
...
agreement with the cue; Appendix F presents this stratified analysis. This level of analysis suggests that adding a cue can often effectively determinize the model's reasoning process.
...
KL-based vs. Advantage-based Importance. Bogdan et al. (2025) propose resampling importance , defined as the KL-divergence between answer distributions at adjacent steps ( st vs. st ◦ at ). Unlike advantage , KL measures overall distributional shifts without indicating if a step at moves the model toward or away from the target answer (a distinction also noted by Nguyen et al. (2025) in response to Du et al. (2024) for factual recall tasks). Further, KL can be inflated by probability mass changes among irrelevant answers. Bogdan et al. (2025) also introduce counterfactual importance , which compares st ◦ at to a distribution where at is excluded via cosine-similarity filtering. However, this approach suffers from high variance in near-deterministic settings, as the number of 'different' samples approaches zero. In contrast, advantage directly targets the outcomes relevant to faithfulness and correctness.
...
- Scott Emmons, Erik Jenner, David K. Elson, Rif A. Saurous, Senthooran Rajamanoharan, HengChen, Irhum Shafkat, and Rohin Shah. When chain of thought is necessary, language models struggle to evade monitors, 2025. URL https://arxiv.org/abs/2507.05246 .
...
- Tamera Lanham, Anna Chen, Ansh Radhakrishnan, Benoit Steiner, Carson Denison, Danny Hernandez, Dustin Li, Esin Durmus, Evan Hubinger, Jackson Kernion, Kamil˙ e Lukoši¯ ut˙ e, Karina Nguyen, Newton Cheng, Nicholas Joseph, Nicholas Schiefer, Oliver Rausch, Robin Larson, Sam McCandlish, Sandipan Kundu, Saurav Kadavath, Shannon Yang, Thomas Henighan, Timothy Maxwell, Timothy Telleen-Lawton, Tristan Hume, Zac HatfieldDodds, Jared Kaplan, Jan Brauner, Samuel R. Bowman, and Ethan Perez. Measuring faithfulness in chain-of-thought reasoning, 2023. URL https://arxiv.org/abs/2307. 13702 .
...
| A Changepoint-Based Labeling of Consequential Steps   | A Changepoint-Based Labeling of Consequential Steps             | 16                                     |
|-------------------------------------------------------|-----------------------------------------------------------------|----------------------------------------|
...
| B Experimental Details                                | B Experimental Details                                          | 21                                     |
| B.1                                                   | Generation Hyperparameters . . . . . . . . . . . . . . .        | . . . . . . . . . . . . . . . . . . 21 |
| B.2                                                   | Judge Details . . . . . . . . . . . . . . . . . . . . . . . . . | . . . . . . . . . . . . . . . . . . 21 |
| B.3                                                   | Fine-Tuning Critic Details . . . . . . . . . . . . . . . . .    | . . . . . . . . . . . . . . . . . . 22 |
| C How Many Reasoning Steps in a Response?             | C How Many Reasoning Steps in a Response?                       | 23                                     |

| D Semantic Step Categories   | D Semantic Step Categories                                     | D Semantic Step Categories                                     | 26                                         |
|------------------------------|----------------------------------------------------------------|----------------------------------------------------------------|--------------------------------------------|
|                              | D.1                                                            | Category Definitions . . . . . . . . . . . . . . . . . .       | . . . . . . . . . . . . . . . . . . . . 26 |
| D.2                          |                                                                | Additional Results . . . . . . . . . . . . . . . . . . .       | . . . . . . . . . . . . . . . . . . . . 26 |
| E                            | Definition of Reasoning Trace Value Patterns                   | Definition of Reasoning Trace Value Patterns                   | 30                                         |
| F                            | Complementing Cue-Based Faithfulness Tests: Additional Details | Complementing Cue-Based Faithfulness Tests: Additional Details | 30                                         |
|                              | F.1                                                            | Experimental Details . . . . . . . . . . . . . . . . . .       | . . . . . . . . . . . . . . . . . . . . 30 |
|                              | F.2                                                            | Stratified Analysis . . . . . . . . . . . . . . . . . . . .    | . . . . . . . . . . . . . . . . . . . . 33 |
| G                            | Judge and Critic Prompt Templates                              | Judge and Critic Prompt Templates                              | 35                                         |
| H                            | Additional Results: Judges and Critics on Non-thinking Models  | Additional Results: Judges and Critics on Non-thinking Models  | 36                                         |
|                              | H.1                                                            | Full Precision-Recall Curves . . . . . . . . . . . . . .       | . . . . . . . . . . . . . . . . . . . . 37 |
|                              | H.2                                                            | AStronger Generator: Qwen3-8B . . . . . . . . . . .            | . . . . . . . . . . . . . . . . . . . . 37 |
|                              | H.3                                                            | Per-Dataset Breakdown . . . . . . . . . . . . . . . .          | . . . . . . . . . . . . . . . . . . . . 39 |
| I                            | Thinking-Model Judge and Critic Results                        | Thinking-Model Judge and Critic Results                        | 39                                         |
| J                            | Results: Correctness-Based Advantage                           | Results: Correctness-Based Advantage                           | 40                                         |
| K                            | Advantage vs Leave-One-Out Advantage                           | Advantage vs Leave-One-Out Advantage                           | 43                                         |
| L                            | Extended Related Work                                          | Extended Related Work                                          | 44                                         |
| M                            | LLM Usage Statement                                            | LLM Usage Statement                                            | 44                                         |
...
with pooled counts K = ∑ t ∈S kt and N = ∑ t ∈S nt and segment-level MLE ˆ p = K / N . The segmentation minimizes the total segment cost plus a penalty λ per boundary, solved exactly by PELT with a minimum segment length of two points. Pooling entire segments on each side of a candidate boundary is what provides statistical power: a single two-proportion comparison at n = 50 rollouts has a worst-case minimum detectable effect of ≈ 0.28 at 80% power, and resolving a jump of 0.1 that way would require n ≈ 392 rollouts per prefix, 9 whereas a segment of 25 steps pools 1,250 rollouts and makes the same jump detectable.

Penalty calibration. The penalty λ is calibrated separately for each (model, thinkingmode, dataset) cell. We simulate 1,000 flat null series per cell, with series lengths resampled from the cell's empirical length distribution, true levels swept over a fixed grid p ∈ { 0.02, 0.05, 0.1, 0.2, . . . , 0.9, 0.95, 0.98 } , and counts kt ∼ Binomial ( 50, p ) . We then select the smallest λ ∈ { 2, 3, 4, 5, 6, 8, 10, 12, 15, 20 } whose per-series false-positive rate (the probability of detecting any boundary on a flat series) is at most 2%. Longer series present more opportunities for a spurious boundary and therefore calibrate to larger penalties: λ = 5 for GSM8K (median series length 11-15 steps), λ = 8 for the thinking-mode AIME cells (median ≈ 300 steps) and the Qwen3-4B AIME 26 cell, and λ = 6 for all remaining cells. At λ = 6, a synthetic clean step of size 0.1 is detected 54% of the time with 10 points per side and 96% of the time with 25; a step of size 0.2 is detected 88% of the time with only 5 points per side and essentially always with 10 or more.

Stage 2: Beta-posterior effect-size filter. The penalty controls whether a boundary exists but says nothing about the size of the jump, so each detected boundary is additionally tested against the effect-size criterion of Section 4. Let ( K b , N b ) and ( K a, N a ) denote the pooled counts of the segments before and after the boundary, computed as in stage 1 ( K = ∑ t ∈S kt rollouts attaining reward 1 out of N = ∑ t ∈S nt total, summed over every prefix in the segment). We place independent posteriors p before ∼ Beta ( 1 + K b , 1 + N b - K b ) and p after ∼ Beta ( 1 + K a, 1 + N a - K a ) (uniform priors) on the two segment levels and draw 10 5 Monte Carlo samples of the jump ∆ p = p after - p before. A boundary passes iff the central 95% credible interval for ∆ p excludes 0 and P ( | ∆ p | &gt; δ ) ≥ 0.95 with δ = 0.1. Under the piecewise-constant model, the segment levels at the boundary step at satisfy p before = V π ( st ) and p after = Q π ( st , at ) , so ∆ p is that step's advantage and this criterion instantiates the definition of a consequential step from Section 4. Unlike the pointwise estimate ˆ A ( st , at ) of Equation (3), however, the posterior for ∆ p pools every prefix in the two flanking segments rather than only the n = 50 rollouts at the two adjacent prefixes; this pooling is what makes jumps near δ certifiable at all. Of the two conditions, only the effect-size requirement is restrictive: the credible interval excludes zero for all but one of the 17,310 boundaries detected under the self-advantage reward (PELT already demands strong evidence of some level change before proposing a boundary), whereas P ( | ∆ p | &gt; δ ) ≥ 0.95 is met by 10,061 of them. The exceedance probabilities also provide a posterior audit of the kept set: a kept boundary is a false discovery if its true jump satisfies | ∆ p | ≤ δ , an event of posterior probability 1 - P ( | ∆ p | &gt; δ ) , so by linearity of expectation the posterior expected number of false discoveries is ∑ ( 1 - P ( | ∆ p | &gt; δ )) over kept boundaries - 16.2 across the 7,563 steps ultimately labeled consequential below, an expected false-discovery rate of ≈ 0.2% under the piecewise-constant model.

9 From δ min ≈ ( z α /2 + z β ) √ 2 p 0 ( 1 - p 0 ) / n (Fleiss et al., 2003) at α = 0.05, evaluated at the worstcase base rate p 0 = 1 2 ; at more extreme base rates (e.g., p 0 = 0.1) the minimum detectable effect shrinks to ≈ 0.17.

Localization requirement and label placement. PELT returns the maximum-likelihood position of each boundary. To quantify how precisely the jump is attributed, we compute a profile-likelihood confidence set per boundary: holding the neighboring boundaries fixed, the boundary is slid across the two flanking segments, and every position whose two-segment cost is within 2 log-likelihood units of the optimum is retained. Because our analyses attach labels to specific sentences, we additionally require this confidence set to be a single step; boundaries with multi-step confidence sets (about 25% of those passing the effect-size filter, typically flanked by short or noisy segments) are discarded rather than assigned to their maximum-likelihood step. The requirements together keep 7,563 of 17,310 detected boundaries for the self-advantage reward (3,600 of 10,369 for correctness): a consequential label asserts jointly that a durable level change exists, that its magnitude exceeds δ with high posterior probability, and that it is localized to exactly that step. Kept jumps are large (median | ∆ p | ≈ 0.31), and calibration on synthetic clean steps shows that, conditional on a singleton confidence set, the maximum-likelihood position coincides with the true jump 80/91/97/99% of the time for jumps of size 0.10/0.15/0.20/0.30 - an expected exact-step precision of ≈ 97-98% over the kept jump-size distribution.
...
2. Value BCE -value regression: binary cross-entropy against the per-prefix value V ( st ) ∈ [ 0, 1 ] , with step advantages derived post-hoc by differencing consecutive value predictions.
...
We call the value of step t high if the Wilson 95% (Wilson, 1927) lower bound of vt exceeds 0.5, i.e., rollouts from this prefix are confidently more likely than not to reach the reference answer ( ˆ vt ≥ 0.64 at n = 50). The patterns are

1. high throughout : every step is high;
2. gradual climb : v 1 is not high, vT is high, and no single-step increase covers half of the total climb, so confidence accrues over many steps;
3. sudden climb : v 1 is not high, vT is high, and a single step increases the value by at least half of the total climb max t vt - min t vt , i.e., one step does the majority of the work;
4. dip &amp; recover : v 1 and vT are both high, but some intermediate step is not;
5. reached high but ends low : some step is high but vT is not, i.e., the model attains and then loses a state from which it would likely have answered correctly;
...
&lt;target&gt;{gold\_target}&lt;/target&gt;

Full response:

&lt;response&gt;{full\_response}&lt;/response&gt;
...
Avg steps Percent Avg steps answering, reason carefully about the counterfactual: how likely was this final answer before the step, and how likely after it? Reserve values near +1 or -1 for steps that single-handedly decide or derail the outcome. Base your estimate only on the question, gold target, full response, and the reasoning step itself, and output a single number in \boxed{} with two decimals (e.g. \boxed{0.05}, \boxed {-0.30}).

This prompt is phrased for self-advantage and is used unchanged for the correctness-based evaluation, where the judge's scores are evaluated against the correctness labels. Finally, the WMSEcritics of Appendix B.3, which regress the advantage directly, are fine-tuned with an advantage-target system prompt (self-advantage form):
...
Generator Model: Qwen3-1.7B (non-thinking) ·
...
Here, we repeat the evaluation of Section 6 for the Qwen3-1.7B model with thinking mode on, training on the AIME 24 dataset (ID) and evaluating out-of-domain on AIME 25. This arm includes a value-derived Qwen3-8B judge, direct-importance judges at all four scales, and think-trained critics at 1.7B, 8B, and Qwen3.6-27B (trained for 2 epochs on 42,000 AIME 24 steps and evaluated on 8,000 AIME 25 steps). 10 Consequential steps are rarer still on thinking traces (prevalence ≈ 0.5% for self-advantage and ≈ 0.2% for correctnessbased advantage), and the split-half ceilings are correspondingly lower ( ≈ 0.3 PR-AUC), so absolute numbers are not comparable to the non-thinking arms.

Figure 24 shows the same qualitative structure as the non-thinking arms, at lower absolute levels. The value-derived judge is at chance (PR-AUC 0.006 ID and 0.004 OOD vs. prevalences of 0.005 and 0.004), replicating the out-of-the-box null result on a second reasoning style. The think-trained critics decode self-advantage well above chance (PR-AUC 0.0370.066 ID, 8-14 × prevalence) but sit further below their ceiling than the non-thinking critics do below theirs.

10 Fine-tuning a 32B critic was infeasible in this arm, given compute constraints at this time.
...
Leave-One-Out (LOO) Advantage Analogous to the counterfactual importance from Bogdan et al. (2025), one might further ask: What would be the difference in value between a prefix st ◦ at and st ◦ a ′ t , where a ′ t = at , called A LOO? This would examine how sampling step at changes the value compared to sampling any other possible step a ′ t . We present a proof that

<!-- formula-not-decoded -->

̸

where ϕ : Σ ∗ → Σ ∗ maps a completion to its first delimited reasoning step, and πϕ ( a | s ) ≜ Pr c ∼ π ( ·| s ) [ ϕ ( c ) = a ] denotes the pushforward distribution over the next reasoning step. That is, the LOO advantage is simply the advantage scaled by the inverse probability of sampling any other reasoning step. However, in practice, judging whether a ′ t = at beyond exact string matching introduces difficult empirical questions. Because of this, we did not empirically test this measure further in this paper.

Preliminaries.

Given state s ∈ Σ ∗ , reasoning step a ∈ Σ ∗ , and the pushforward distribution πϕ ( a | s ) as defined above, recall that the advantage and counterfactual advantage are:
...
(We divide by 1 - ( a | s ) because this is conditional on not choosing a .)
